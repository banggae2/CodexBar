        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .firstTextBaseline, spacing: 12) {
                VStack(alignment: .leading, spacing: 4) {
                    Text(L10n.string(self.toggle.title))
                        .font(.subheadline.weight(.semibold))
                    Text(L10n.string(self.toggle.subtitle))
                        .font(.footnote)
                ...
                if !actions.isEmpty {
                    HStack(spacing: 10) {
                        ForEach(actions) { action in
                            Button(L10n.string(action.title)) {
                                Task { @MainActor in
    ...
    var body: some View {
        ...
        VStack(alignment: .leading, spacing: 6) {
            HStack(alignment: .firstTextBaseline, spacing: 10) {
                Text(L10n.string(self.picker.title))
                    .font(.subheadline.weight(.semibold))
                ...
                Picker("", selection: self.picker.binding) {
                    ForEach(self.picker.options) { option in
                        Text(L10n.string(option.title)).tag(option.id)
                    }
                }
                .labelsHidden()
                .pickerStyle(.menu)
...
struct ProviderSettingsFieldRowView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if hasHeader {
                VStack(alignment: .leading, spacing: 4) {
                    if !trimmedTitle.isEmpty {
                        Text(L10n.string(trimmedTitle))
                            .font(.subheadline.weight(.semibold))
                    }
                    if !trimmedSubtitle.isEmpty {
                        Text(L10n.string(trimmedSubtitle))
                            .font(.footnote)
                            .foregroundStyle(.secondary)
                            .fixedSize(horizontal: false, vertical: true)
                    }
            ...
            case .plain:
                TextField(L10n.string(self.field.placeholder ?? ""), text: self.field.binding)
                    .textFieldStyle(.roundedBorder)
                    .font(.footnote)
                    .onTapGesture { self.field.onActivate?() }
            case .secure:
                SecureField(L10n.string(self.field.placeholder ?? ""), text: self.field.binding)
                    .textFieldStyle(.roundedBorder)
                    .font(.footnote)
                    .onTapGesture { self.field.onActivate?() }
            }
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            HStack(alignment: .center, spacing: 12) {
                Text(L10n.string(self.descriptor.title))
                    .font(.subheadline.weight(.semibold))
                Spacer(minLength: 8)
            ...
            if !self.descriptor.subtitle.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                Text(L10n.string(self.descriptor.subtitle))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
            ...
            if accounts.isEmpty {
                Text(L10n.string("No token accounts yet."))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            } else {
                VStack(alignment: .leading, spacing: 8) {
            ...
            if self.descriptor.primaryAddAction == nil {
                HStack(spacing: 8) {
                    TextField(L10n.string("Label"), text: self.$newLabel)
                        .textFieldStyle(.roundedBorder)
                        .font(.footnote)
                    SecureField(L10n.string(self.descriptor.placeholder), text: self.$newToken)
                        .textFieldStyle(.roundedBorder)
                        .font(.footnote)
            ...
            HStack(spacing: 10) {
                Button(L10n.string("Open token file")) {
