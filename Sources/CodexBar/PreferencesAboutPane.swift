    ...
    var body: some View {
        VStack(spacing: 12) {
            ...
            VStack(spacing: 2) {
                Text("CodexBar")
                    .font(.title3).bold()
                Text(L10n.string("Version %@", self.versionString))
                    .foregroundStyle(.secondary)
                if let buildTimestamp {
                    Text(L10n.string("Built %@", buildTimestamp))
                        .font(.footnote)
                        .foregroundStyle(.secondary)
                }
                Text(L10n.string("May your tokens never run out—keep agent limits in view."))
                    .font(.footnote)
                    .foregroundStyle(.secondary)
            }
            ...
            VStack(alignment: .center, spacing: 10) {
                ...
                AboutLinkRow(icon: "globe", title: L10n.string("Website"), url: "https://steipete.me")
                AboutLinkRow(icon: "bird", title: L10n.string("Twitter"), url: "https://twitter.com/steipete")
                AboutLinkRow(icon: "envelope", title: L10n.string("Email"), url: "mailto:peter@steipete.me")
            }
            .padding(.top, 8)
            ...
            if self.updater.isAvailable {
                VStack(spacing: 10) {
                    Toggle(L10n.string("Check for updates automatically"), isOn: self.$autoUpdateEnabled)
                        .toggleStyle(.checkbox)
                        .frame(maxWidth: .infinity, alignment: .center)
                    VStack(spacing: 6) {
                        HStack(spacing: 12) {
                            Text(L10n.string("Update Channel"))
                            Spacer()
                    ...
                    Button(L10n.string("Check for Updates…")) { self.updater.checkForUpdates(nil) }
                }
            } else {
                Text(self.updater.unavailableReason ?? L10n.string("Updates unavailable in this build."))
                    .foregroundStyle(.secondary)
            }

            Text(L10n.string("© 2026 Peter Steinberger. MIT License."))
                .font(.footnote)
                .foregroundStyle(.secondary)
                .padding(.top, 4)
