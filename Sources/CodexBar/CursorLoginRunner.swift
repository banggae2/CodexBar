...
final class CursorLoginRunner: NSObject {
    ...
    private func setupWindow() {
        ...
        window.title = L10n.string("Cursor Login")
    ...
    private func captureSessionCookies() async {
        ...
        guard !cursorCookies.isEmpty else {
            self.phaseCallback?(.failed(L10n.string("No session cookies found")))
            self.logger.warning("Cursor login failed: no session cookies found")
            self.complete(with: Result(outcome: .failed(L10n.string("No session cookies found")), email: nil))
            return
        }
