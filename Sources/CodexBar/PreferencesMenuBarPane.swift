...
struct MenuBarPane: View {
        ...
        Form {
            Section {
                ...
                SettingsMenuPicker(
                    ...
                    options: MenuBarUsageDisplayStyle.allCases,
                    label: {
                        SettingsRowLabel(
                            L("Usage display style"),
                            subtitle: L("Choose how usage appears in the menu bar."))
                    },
                    optionLabel: { style in
                        Text(style.label)
                    })
    ...
    private var compactProviderRow: some View {
        LabeledContent {
            ...
            SettingsRowLabel(L("Compact bar providers"), subtitle: self.compactProviderSubtitle)
        }
    }

    private var compactProviderSubtitle: String {
        if self.settings.menuBarIconStyle != .iconAndPercent ||
            self.settings.menuBarUsageDisplayStyle != .compactBars
        {
            L("Select Compact bars to choose which providers appear there.")
        } else if self.activeProvidersInOrder.isEmpty {
            L("No enabled providers available for compact bars.")
        } else {
            self.compactProviderSelectionSummary
        }
    }
