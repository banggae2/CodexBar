...
public enum UsageFormatter {
        ...
        now: Date = .init()) -> String?
    {
        if let date = window.resetsAt {
            ...
            case .both:
                let absolute = self.resetDescription(from: date, now: now)
                let combined = self.localized("Reset combined format", countdown, absolute)
                let text = combined == "Reset combined format" ? "\(countdown) (\(absolute))" : combined
                return self.localized("Resets %@", text)
            }
        }
