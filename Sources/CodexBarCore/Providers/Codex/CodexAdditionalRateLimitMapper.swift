...
enum CodexAdditionalRateLimitMapper {
    ...
    {
        if entry.usesExplicitWindowTitles {
            return self.explicitWindowTitles(from: entry, usedIDs: &usedIDs, now: now)
        }
    ...
    private static func explicitWindowTitles(
        from entry: CodexUsageResponse.AdditionalRateLimit,
        usedIDs: inout Set<String>,
        now: Date) -> [NamedRateWindow]
    {
        guard let titleBase = self.firstNonEmpty(entry.limitName, entry.meteredFeature) else { return [] }
        let baseID = self.slug(titleBase)
        guard !baseID.isEmpty else { return [] }

        let candidates: [(CodexUsageResponse.WindowSnapshot?, suffix: String, titleSuffix: String)] = [
            (entry.rateLimit?.primaryWindow, "5h", "5h"),
            (entry.rateLimit?.secondaryWindow, "weekly", "weekly"),
        ]

        return candidates.compactMap { snapshot, suffix, titleSuffix in
